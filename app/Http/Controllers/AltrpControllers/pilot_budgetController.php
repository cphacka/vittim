...
class pilot_budgetController extends ApiController
{
    ...
    public function pilot_budgets(ApiRequest $request)
    {
        $entity = Model::where('name', 'pilot_budget')->first()->altrp_sql_editors->where('name', 'pilot_budgets')->first();

        $res = selectForSQLEditor(
        "select `p`.* from `vit_pilot_budgets` as `p`
where `p`.`pilot_id` = " . request()->id . "",  [], [
           'sql_name' => 'pilot_budgets',
           'table_name' => 'pilot_budgets',
         ], $request );
