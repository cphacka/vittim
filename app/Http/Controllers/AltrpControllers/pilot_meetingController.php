...
class pilot_meetingController extends ApiController
{
    ...
    public function pilot_meetings(ApiRequest $request)
    {
        $entity = Model::where('name', 'pilot_meeting')->first()->altrp_sql_editors->where('name', 'pilot_meetings')->first();

        $res = selectForSQLEditor(
        "select `p`.* from `vit_pilot_meetings` as `p`
where `p`.`pilot_project_id` = " . request()->id . "",  [], [
           'sql_name' => 'pilot_meetings',
           'table_name' => 'pilot_meetings',
         ], $request );
