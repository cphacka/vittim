...
class pilot_identificationController extends ApiController
{
    ...
    public function pilot_identifications(ApiRequest $request)
    {
        $entity = Model::where('name', 'pilot_identification')->first()->altrp_sql_editors->where('name', 'pilot_identifications')->first();

        $res = selectForSQLEditor(
        "select `p`.* from `vit_pilot_identifications` as `p`
where `p`.`pilot_project_id` = " . request()->id . "",  [], [
           'sql_name' => 'pilot_identifications',
           'table_name' => 'pilot_identifications',
         ], $request );
