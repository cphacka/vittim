...
class pilot_teamController extends ApiController
{
    ...
    public function pilot_teams(ApiRequest $request)
    {
        $entity = Model::where('name', 'pilot_team')->first()->altrp_sql_editors->where('name', 'pilot_teams')->first();

        $res = selectForSQLEditor(
        "SELECT `con`.`fio`, `con`.`position`, `con`.`email`
FROM `vit_pilot_teams` as `pt`
LEFT JOIN `vit_contacts` as `con` on `pt`.`contact_id`=`con`.`id`
where `pt`.`pilot_id` = " . request()->id . "",  [], [
           'sql_name' => 'pilot_teams',
           'table_name' => 'pilot_teams',
         ], $request );
