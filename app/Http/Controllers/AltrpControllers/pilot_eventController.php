...
class pilot_eventController extends ApiController
{
    ...
    public function pilot_events(ApiRequest $request)
    {
        $entity = Model::where('name', 'pilot_event')->first()->altrp_sql_editors->where('name', 'pilot_events')->first();

        $res = selectForSQLEditor(
        "select `p`.* from `vit_pilot_events` as `p`
where `p`.`pilot_id` = " . request()->id . "",  [], [
           'sql_name' => 'pilot_events',
           'table_name' => 'pilot_events',
         ], $request );
