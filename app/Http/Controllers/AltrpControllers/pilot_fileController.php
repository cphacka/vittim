...
class pilot_fileController extends ApiController
{
    ...
    public function pilot_files(ApiRequest $request)
    {
        $entity = Model::where('name', 'pilot_file')->first()->altrp_sql_editors->where('name', 'pilot_files')->first();

        $res = selectForSQLEditor(
        "select `p`.* from `vit_pilot_files` as `p`
where `p`.`pilot_project_id` = " . request()->id . "",  [], [
           'sql_name' => 'pilot_files',
           'table_name' => 'pilot_files',
         ], $request );
